...
public class CameraManager : Singleton<CameraManager>
{
    ...
    public void InitLimitPosition()
    {
        ...
        if (SpawnManager.Ins.arrowTiles.Count > 0)
        {
            // Khởi tạo giá trị ban đầu từ phần tử đầu tiên
            minX = maxX = SpawnManager.Ins.arrowTiles[0].transform.position.x;
            minY = maxY = SpawnManager.Ins.arrowTiles[0].transform.position.y;

            // Lặp qua từng phần tử trong danh sách
            foreach (var tile in SpawnManager.Ins.arrowTiles)
            {
